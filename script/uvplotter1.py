...
class ApSynSim_UV_Plotter1(object):
    ...
    def __init__(self, parent):
        ...
        self.canvasUV1 = FigureCanvasTkAgg(
            self.figUV1, master=self.frames['FigUV'])
    ...
    def _FFTRead(self):
        ...
        vmax = np.max(Toplot)
        self.UVCLEANMODPlot = self.UVCLEANMOD.imshow(
            Toplot, vmin=0.0, vmax=vmax,
                                                     cmap=self.parent.currcmap,
                                                     picker=True,
                                                     interpolation='nearest')
    ...
    def _onUVPick(self, event):
        ...
        if xi > 0 and yi > 0 and xi < self.parent.Npix and yi < self.parent.Npix:
            ...
            self.ResidText.set_text(
                self.UVfmt % self.UVResidPlot.get_array()[xi, yi])
            self.CLEANText.set_text(
                self.UVfmt % self.UVCLEANPlot.get_array()[xi, yi])
            self.CLEANMODText.set_text(
                self.UVfmt % self.UVCLEANMODPlot.get_array()[xi, yi])
